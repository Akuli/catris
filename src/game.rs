...
use crate::modes::TraditionalMode;
...
#[impl_enum::with_methods {
    ...
    fn find_full_rows(&self) -> Vec<WorldPoint> {}
    fn remove_full_rows_raw(&mut self, full_points: &[WorldPoint]) {}
}]
enum ModeSpecific {
    Traditional(TraditionalMode),
}
...
impl Game {
    pub fn new(mode: Mode) -> Self {
        Self {
            mode_specific: match mode {
                Mode::Traditional => ModeSpecific::Traditional(TraditionalMode::new()),
    ...
    pub fn mode(&self) -> Mode {
        match &self.mode_specific {
            ModeSpecific::Traditional(_) => Mode::Traditional,
        }
    }

    pub fn add_player(&mut self, client_info: &ClientInfo) {
        let player_index = self.get_players().len();
        match &mut self.mode_specific {
            ModeSpecific::Traditional(game) => game.add_player(client_info),
        }
    ...
    fn get_players(&self) -> &Vec<RefCell<Player>> {
        match &self.mode_specific {
            ModeSpecific::Traditional(game) => &game.players,
        }
    }
    ...
    pub fn get_flashing_points(&self) -> &HashMap<WorldPoint, u8> {
        match &self.mode_specific {
            ModeSpecific::Traditional(game) => &game.flashing_points,
        }
    }

    pub fn get_flashing_points_mut(&mut self) -> &mut HashMap<WorldPoint, u8> {
        match &mut self.mode_specific {
            ModeSpecific::Traditional(game) => &mut game.flashing_points,
        }
    }

    pub fn find_full_rows(&self) -> Vec<WorldPoint> {
        self.mode_specific.find_full_rows()
    }

    pub fn remove_player_if_exists(&mut self, client_id: u64) {
        self.mode_specific.remove_player_if_exists(client_id);
    }

    fn square_is_occupied(&self, point: WorldPoint, exclude_player_idx: Option<usize>) -> bool {
        (self.mode_specific.is_valid_landed_block_coords(point) && self.mode_specific.get_landed_square(point).is_some())
    ...
    fn rotate_if_possible(&self, player_idx: usize) -> bool {
        ...
        let can_rotate = !coords.is_empty()
            && coords.iter().all(|p| {
                let stays_in_bounds = self.mode_specific.is_valid_moving_block_coords(*p);
    ...
    pub fn render_to_buf(&self, client_id: u64, buffer: &mut render::Buffer) {
        ...
        let (offset_x, offset_y) = self.mode_specific.render_world_edges_to_buf(buffer);
        let trace_points = self.predict_landing_place(player_idx);

        // TODO: optimize lol?
        for x in i8::MIN..i8::MAX {
            for y in i8::MIN..i8::MAX {
                if !self.mode_specific.is_valid_landed_block_coords((x, y)) {
                    continue;
                }
                ...
                if content.is_none() {
                    let mut traced_content =
                        self.mode_specific.get_landed_square((x, y)).unwrap_or(SquareContent {
                            text: [' ', ' '],
                            color: Color::DEFAULT,
                        });
    ...
    pub fn move_blocks_down(&mut self, fast: bool) -> bool {
        ...
        for (player_idx, player) in self.get_players().iter().enumerate() {
            ...
            if world_points
                .iter()
                .all(|p| self.mode_specific.is_valid_landed_block_coords(*p))
            {
                // land the block
        ...
        for (point, content) in landing {
            self.mode_specific.set_landed_square(point, Some(content));
        }
    ...
    pub fn remove_full_rows(&mut self, full: &[WorldPoint]) {
        self.mode_specific.remove_full_rows_raw(full);
        ...
        for point in potential_overlaps {
            if self.mode_specific.is_valid_landed_block_coords(point) {
                self.mode_specific.set_landed_square(point, None);
            }
        }
    }
