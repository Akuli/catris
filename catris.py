...
class GameState:
    ...
    def end_waiting(self, player: Player, client_connected: bool) -> None:
        assert player.moving_block_or_wait_counter == 0
        if self.game_is_over() or not client_connected:
        ...
        while True:
            ...
            with self.access_game_state() as state:
                ...
                if player.moving_block_or_wait_counter == 0:
                    state.end_waiting(
                        player,
                        client_connected=any(c.player == player for c in self.clients),
                    )
                    return
